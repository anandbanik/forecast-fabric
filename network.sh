...
CHAINCODE_WALMART_UNILEVER=forecast
CHAINCODE_WALMART_UNILEVER_INIT='{"Args":["init"]}'
...
DEFAULT_API_EXTRA_HOSTS1="extra_hosts:[newline]      - orderer.$DOMAIN:$IP_ORDERER[newline]      - peer0.$ORG1.$DOMAIN:$IP1[newline]      - peer0.$ORG2.$DOMAIN:$IP2[newline]"
...
function installWalmartUnilever() {
  org=$1

  for chaincode_name in ${CHAINCODE_WALMART_UNILEVER}
  do
...
if [ "${MODE}" == "up" -a "${ORG}" == "" ]; then
  ...
  for org in ${ORG1} ${ORG2}
  do
    installWalmartUnilever ${org}
  done
  
elif [ "${MODE}" == "join" -a "${ORG}" == "" ]; then
  createJoinInstantiateWarmUp ${ORG1} "${ORG1}-${ORG2}" ${CHAINCODE_WALMART_UNILEVER} ${CHAINCODE_WALMART_UNILEVER_INIT}
  joinWarmUp ${ORG2} "${ORG1}-${ORG2}" ${CHAINCODE_WALMART_UNILEVER} ${CHAINCODE_WALMART_UNILEVER_INIT}
